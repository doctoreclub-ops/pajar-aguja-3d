...
export default wrap(async (req, res) => {
  ...
  if (t > Date.now() + 5 * 60e3) return send(res, 400, { ok: false, error: 'clock' });
  ...
  if (since < (final ? Math.min(5, MIN_INTERVAL) : MIN_INTERVAL - 1)) return send(res, 429, { ok: false, error: 'soon', retryIn: Math.ceil(MIN_INTERVAL - since) });
